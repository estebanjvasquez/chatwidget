(function() {
    ...
    function initWidget() {
        ...
        const styles = `
            ...
            /* Debug Style for Raw JSON */
            .n8n-chat-msg.bot code { background: #eee; padding: 2px 4px; border-radius: 4px; font-family: monospace; font-size: 0.9em; display: block; white-space: pre-wrap; word-break: break-all; }
        ...
        function addMsg(text, type) {
            ...
            div.innerHTML = text; // HTML enabled
        ...
        async function sendMsg() {
            ...
            try {
                ...
                console.log("N8N DEBUG:", data); // Check Console F12

                let reply = "";

                // --- DEBUGGING LOGIC ---
                // 1. Look for array output
                if (Array.isArray(data) && data.length > 0) {
                    reply = data[0].output || data[0].text || data[0].message;
                } 
                // 2. Look for object output
                else if (typeof data === 'object') {
                    reply = data.output || data.text || data.message;
                }

                // 3. IF EMPTY -> SHOW RAW DATA (Debugging)
                if (!reply) {
